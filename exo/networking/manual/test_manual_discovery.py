...
root_path = "./exo/networking/manual/"
...
class TestManualDiscovery(unittest.IsolatedAsyncioTestCase):
  async def asyncSetUp(self):
    ...
    self.discovery1 = ManualDiscovery(root_path + "test_data/test_config.json", "node1", create_peer_handle=lambda peer_id, address, device_capabilities: self.peer1)
    self.discovery2 = ManualDiscovery(root_path + "test_data/test_config.json", "node2", create_peer_handle=lambda peer_id, address, device_capabilities: self.peer2)
...
class TestManualDiscoveryWithGRPCPeerHandle(unittest.IsolatedAsyncioTestCase):
  async def asyncSetUp(self):
    config = NetworkTopology.from_path(root_path + "test_data/test_config.json")
    print(config)
    ...
    self.discovery1 = ManualDiscovery(
      root_path + "test_data/test_config.json", "node1", create_peer_handle=lambda peer_id, address, device_capabilities: GRPCPeerHandle(peer_id, address, device_capabilities)
    )
    self.discovery2 = ManualDiscovery(
      root_path + "test_data/test_config.json", "node2", create_peer_handle=lambda peer_id, address, device_capabilities: GRPCPeerHandle(peer_id, address, device_capabilities)
    )
    await self.discovery1.start()
    await self.discovery2.start()
